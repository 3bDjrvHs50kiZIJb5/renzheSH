...
canshu_v6() {
	if grep -q '^canshu="V6"' /usr/local/bin/r > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/renzhe.sh
	elif grep -q '^canshu="V6"' ~/renzhe.sh.bak > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/renzhe.sh
	fi
}
...
CheckFirstRun_true() {
	if grep -q '^permission_granted="true"' /usr/local/bin/r > /dev/null 2>&1; then
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/renzhe.sh
	elif grep -q '^permission_granted="true"' ~/renzhe.sh.bak > /dev/null 2>&1; then
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/renzhe.sh
	fi
}
...
if grep -q '^ENABLE_STATS="false"' /usr/local/bin/r > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/renzhe.sh
elif grep -q '^ENABLE_STATS="false"' ~/renzhe.sh.bak > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/renzhe.sh
fi
...
install_ldnmp() {
	  ...
	  # mysql调优
	  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/custom_mysql_config-1.cnf
...
install_certbot() {
	...
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/auto_cert_renewal.sh
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  1)
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/fail2ban-nginx-cc.conf
				  ...
				  22)
					  ...
					  install jq bc
					  check_crontab_installed
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/CF-Under-Attack.sh
...
web_optimization() {
		  while true; do
			  ...
			  case $sub_choice in
				  ...
				  # php调优
				  wget -O /home/optimized_php.ini ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/optimized_php.ini
				  docker cp /home/optimized_php.ini php:/usr/local/etc/php/conf.d/optimized_php.ini
				  docker cp /home/optimized_php.ini php74:/usr/local/etc/php/conf.d/optimized_php.ini
				  rm -rf /home/optimized_php.ini

				  # php调优
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/www-1.conf
				  ...
				  # php调优
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/www.conf
				  docker cp /home/www.conf php:/usr/local/etc/php-fpm.d/www.conf
				  docker cp /home/www.conf php74:/usr/local/etc/php-fpm.d/www.conf
				  rm -rf /home/www.conf
				  ...
				  # mysql调优
				  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/custom_mysql_config.cnf
				  docker cp /home/custom_mysql_config.cnf mysql:/etc/mysql/conf.d/
				  rm -rf /home/custom_mysql_config.cnf
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://github.com/cenet999/sh"
  	  echo "------------------------------------------------"
			...
			while true; do
				  ...
				  case $sub_choice in
					  1)
						...
						wget -qO - ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes

						# 步骤3：添加存储库
						echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list

						# version=$(wget -q https://dl.xanmod.org/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
						local version=$(wget -q ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  6)
			  ...
			  clear
			  restore_defaults
			  curl -sS ${gh_proxy}raw.githubusercontent.com/cenet999/sh/refs/heads/main/network-optimize.sh -o /tmp/network-optimize.sh && source /tmp/network-optimize.sh && restore_network_defaults
			  send_stats "还原默认设置"
			  ;;
		  ...
		  7)
			  cd ~
			  clear
			  curl -sS ${gh_proxy}raw.githubusercontent.com/cenet999/sh/refs/heads/main/network-optimize.sh | bash
			  send_stats "内核自动调优"
			  ;;
...
delete_task_schedule() {
	...
	crontab -l | grep -v "r rsync_run $num" | crontab -
...
linux_ldnmp() {
  while true; do
	...
	33)
	  ...
	  cd ~
	  wget -O ${useip}_beifen.sh ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/beifen.sh > /dev/null 2>&1
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}117. ${color117}SQL Server数据库服务                ${gl_kjlan}118. ${color118}Nginx静态应用站"
	  echo -e "${gl_kjlan}-------------------------"
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  115|v2raya|V2RayA|v2rayA)
			...
			local app_id="115"
			local app_name="V2RayA代理管理面板"
			local app_text="一个自托管代理管理面板，适合用网页管理 v2ray/xray 节点和系统代理。"
			local app_url="项目地址: ${gh_https_url}github.com/v2rayA/v2rayA"
	  ...
	  116|shadowsocks-rust|ssserver-rust|shadowsocks)
			...
			local app_id="116"
			local app_name="Shadowsocks Rust代理服务端"
			local app_text="轻量代理服务，适合给手机客户端或电脑客户端直连使用。它不是网页面板，客户端填服务器IP、端口、密码、加密方式即可。"
			local app_url="项目地址: ${gh_https_url}github.com/shadowsocks/shadowsocks-rust"
			...
			docker_app_install() {
				...
				read -e -p "设置 Shadowsocks 密码，回车自动生成随机密码: " ss_password
				local ss_password=${ss_password:-$(tr -dc 'A-Za-z0-9' < /dev/urandom | head -c 16)}

				read -e -p "设置加密方式，回车默认 chacha20-ietf-poly1305: " ss_method
...
EOF
			...
			docker_app_update() {
				...
				clear
				echo "更新完成"
				echo "当前客户端参数保存在: ${app_workdir}/client-info.txt"
			}
...
EOF
		...
		docker_app_plus
		  ...
		  118|nginx-static|nginx|static-nginx)
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载忍者工具箱"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
EOF
			...
			while true; do
				...
				case "$Limiting" in
				  1)
					...
					cd ~
					curl -Ss -o ~/Limiting_Shut_down.sh ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/Limiting_Shut_down1.sh
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  if [ -f ~/TG-check-notify.sh ]; then
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/TG-check-notify.sh
					  chmod +x ~/TG-check-notify.sh
					  nano ~/TG-check-notify.sh
				  fi
				  ...
				  curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/TG-SSH-check-notify.sh > /dev/null 2>&1
		  ...
		  26)
			  ...
			  cd ~
			  curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/upgrade_openssh9.8p1.sh
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "访问忍者工具箱官方留言板，您对脚本有任何想法欢迎留言交流！"
			...
			while true; do
			  ...
			  case $sub_choice in
				  1)
					  ...
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' ~/renzhe.sh
					  echo "已开启采集"
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装忍者工具箱         ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
	  ...
	  case $sub_choice in
		  ...
		  11)
			  ...
			  ;;
		  12)
			  run_commands_on_servers "r update"
			  ;;
		  13)
			  run_commands_on_servers "r clean"
			  ;;
		  14)
			  run_commands_on_servers "r docker install"
			  ;;
		  15)
			  run_commands_on_servers "r bbr3"
			  ;;
		  16)
			  run_commands_on_servers "r swap 1024"
			  ;;
		  17)
			  run_commands_on_servers "r time Asia/Shanghai"
			  ;;
		  18)
			  run_commands_on_servers "r iptables_open"
			  ;;
...
echo -e "${gl_kjlan}博客: ${gl_bai}https://github.com/cenet999/sh         ${gl_kjlan}软件中心: ${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://github.com/cenet999/sh            ${gl_kjlan}GitHub地址: ${gl_bai}https://github.com/cenet999/sh${gl_bai}"
echo "------------------------"
echo ""
}
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  1) send_stats "幻兽帕鲁开服脚本" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/palworld.sh ; chmod +x palworld.sh ; ./palworld.sh
			 exit
			 ;;
		  2) send_stats "我的世界开服脚本" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/cenet999/sh/main/mc.sh ; chmod +x mc.sh ; ./mc.sh
			 exit
			 ;;
...
while true; do
	...
	case "$choice" in
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				cron_proxy="https://"
				cron_sed_cmd="sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ~/renzhe.sh"
			elif [ -n "$ipv6_address" ]; then
				cron_proxy="https://"
				cron_sed_cmd="sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ~/renzhe.sh"
			else
				cron_proxy="https://"
				cron_sed_cmd=""
			fi
...
echo -e "${gl_kjlan}3.   ${gl_bai}站点数据管理 ${gl_huang}★${gl_bai}"
echo -e "${gl_kjlan}4.   ${gl_bai}站点反向代理，IP加端口"
echo -e "${gl_kjlan}5.   ${gl_bai}站点反向代理，IP加端口，无SSL"
echo -e "${gl_kjlan}6.   ${gl_bai}应用市场"
echo -e "${gl_kjlan}7.   ${gl_bai}CodeX CLI(API)"
echo -e "${gl_kjlan}8.   ${gl_bai}V2Ray-Agent安装脚本"
echo -e "${gl_kjlan}9.   ${gl_bai}旧的菜单"
echo -e "${gl_kjlan}------------------------${gl_bai}"
...
case $choice in
  ...
  3) ldnmp_web_status ;;
  4) ldnmp_Proxy ;;
  5) ldnmp_Proxy "" "" "" "no_ssl" ;;
  6) linux_panel ;;
  7) codex_cli_menu ;;
  8) v2ray_agent_install ;;
  9) kejilion_legacy_menu ;;
  0) clear ; exit ;;
...
echo "╚═╗║  ║  ║ ║╠╣ ╚═╗"
echo "╚═╝╚═╝╩═╝╚═╝╚  ╚═╝"
echo -e "CodeX CLI(API) 菜单"
echo -e "${gl_kjlan}------------------------${gl_bai}"
echo -e "${gl_kjlan}1.   ${gl_bai}查看当前配置"
echo -e "${gl_kjlan}2.   ${gl_bai}安装/升级 CodeX CLI"
echo -e "${gl_kjlan}3.   ${gl_bai}配置 API 模式"
echo -e "${gl_kjlan}4.   ${gl_bai}启动 CodeX CLI"
echo -e "${gl_kjlan}------------------------${gl_bai}"
echo -e "${gl_kjlan}0.   ${gl_bai}返回根菜单"
echo -e "${gl_kjlan}------------------------${gl_bai}"
read -e -p "请输入你的选择: " choice
...
case $choice in
  1) codex_cli_show_status ; break_end ;;
  2) codex_cli_install_or_upgrade ; break_end ;;
  3) codex_cli_config_api_mode ; break_end ;;
  4) codex_cli_launch ; break_end ;;
  0) return ;;
  *) echo "无效的输入!" ; break_end ;;
esac
done
...
if cfg.exists():
    try:
        ...
        print(f"ask_for_approval: {data.get('ask_for_approval', '')}")
        print(f"sandbox_mode: {data.get('sandbox_mode', '')}")
        provider = data.get('model_providers', {}).get('OpenAI', {})
        print(f"base_url: {provider.get('base_url', '')}")
        print(f"provider_name: {provider.get('name', '')}")
        print(f"requires_openai_auth: {provider.get('requires_openai_auth', '')}")
        print(f"wire_api: {provider.get('wire_api', '')}")
else:
    print("配置状态: 未找到 config.toml")

if auth.exists():
    try:
        payload = json.loads(auth.read_text(encoding="utf-8"))
    except Exception as exc:
        print(f"OPENAI_API_KEY 状态: 读取失败({exc})")
    else:
        print(f"OPENAI_API_KEY: {'已设置（已打码）' if payload.get('OPENAI_API_KEY') else '未设置'}")
else:
...
codex_cli_read_current() {
	python3 - "$HOME/.codex/config.toml" "$HOME/.codex/auth.json" <<'PY'
from pathlib import Path
import json
import shlex
import sys
...
if cfg.exists():
    try:
        ...
        data = {}
    provider = data.get("model_provider", provider) or provider
    model = data.get("model", "") or ""
    reasoning = data.get("model_reasoning_effort", "") or ""
    openai = data.get("model_providers", {}).get("OpenAI", {})
    base_url = openai.get("base_url", "") or ""
...
if auth.exists():
    try:
        ...
        payload = {}
    key_set = "yes" if payload.get("OPENAI_API_KEY") else "no"

print(f"CODEx_CURRENT_PROVIDER={shlex.quote(provider)}")
print(f"CODEx_CURRENT_BASE_URL={shlex.quote(base_url)}")
print(f"CODEx_CURRENT_MODEL={shlex.quote(model)}")
print(f"CODEx_CURRENT_REASONING={shlex.quote(reasoning)}")
print(f"CODEx_CURRENT_KEY_SET={shlex.quote(key_set)}")
PY
}


codex_cli_update_config_file() {
	local provider="$1"
	local base_url="$2"
	local model="$3"
	local reasoning="$4"
	...
	mkdir -p "$HOME/.codex"
	python3 - "$cfg" "$provider" "$base_url" "$model" "$reasoning" <<'PY'
from pathlib import Path
import re
import sys
...
def replace_or_add(text, key, value, quote=True):
    if quote:
        line = f'{key} = "{value}"'
    else:
        line = f'{key} = {value}'
...
text = replace_or_add(text, "model_provider", provider)
text = replace_or_add(text, "model", model)
text = replace_or_add(text, "model_reasoning_effort", reasoning)
...
codex_cli_config_api_mode() {
	...
	current_values="$(codex_cli_read_current)"
	eval "$current_values"

	provider_name="${CODEx_CURRENT_PROVIDER:-OpenAI}"
	base_url="${CODEx_CURRENT_BASE_URL:-https://token.renzhe.org/v1}"
	model="${CODEx_CURRENT_MODEL:-gpt-5.4-mini}"
	reasoning="${CODEx_CURRENT_REASONING:-medium}"
	...
	echo "API Key: $([ "${CODEx_CURRENT_KEY_SET:-no}" = "yes" ] && echo "已设置（已打码）" || echo "未设置")"
	echo ""

	read -e -p "Provider 名称 [${provider_name}]: " input_provider
	provider_name="${input_provider:-$provider_name}"

	read -e -p "Base URL [${base_url}]: " input_base_url
	base_url="${input_base_url:-$base_url}"

	read -e -p "模型 [${model}]: " input_model
...
api_key = sys.argv[2]
auth.write_text(json.dumps({"OPENAI_API_KEY": api_key}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
PY
		echo "API Key 已写入 ${auth}"
	elif [ "${CODEx_CURRENT_KEY_SET:-no}" != "yes" ]; then
		echo "未输入 API Key，且当前也没有检测到已保存的 Key。"
		return 1
	fi
